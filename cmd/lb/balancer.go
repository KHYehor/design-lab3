...
var (
	...
	// This is traffic server's traffic volume
	serversPoolTraffic = []int64{
		0,
		0,
		0,
	}
	// This is health status of server. Are they alive
	serverStatus = []bool {
		true,
		true,
		true,
...
// Function for getting the server with minimal traffic volume
func getTheBestServer() (string, error) {
	var theBestServerIndex = -1
	// This looks like magic number - but it is int64 maximum number
	var minimalTraffic int64 = 9223372036854775807
	for i := 0; i < 3; i++ {
		// Checking for alive and the minimal server
...
func health(dst string, index int) {
	...
	// Parsing traffic volume about server
...
func main() {
	flag.Parse()
	// Refreshing history of traffic volume
	go func() {
	...
	// Writing info about server's traffic to pool
	...
	frontend := httptools.CreateServer(*port, http.HandlerFunc(func(rw http.ResponseWriter, r *http.Request) {
		// getting the best server for redirection
